...
8. **Build firmware** — `yes "" | make -j$(nproc) V=s`（`yes ""` 防止内核 syncconfig 在 CI 无终端环境中进入交互模式）
...
- `devices/<name>/customize.sh` — 设备专属，复制到 `openwrt/device-files/` 后在 openwrt/ 内执行（feeds 之前）。JDCloud AX3000 用它以 `patch -p1` 循环应用 `device-files/*.patch`；Redmi / x86_64 目前是空模板
...
└── *.patch                   # 可选设备补丁，由 customize.sh 用 patch -p1 应用
.github/workflows/
...
- config 来自本地能成功编译运行的构建目录（`../lede/.config`、`../lienol/.config`、`../libwrt/.config`），分别对应 x86_64 / JDCloud AX3000 / Redmi AX5 的源码
