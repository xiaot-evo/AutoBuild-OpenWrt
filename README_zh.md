...
- 设备专属 `devices/<name>/customize.sh`：在克隆的源码树内、feeds 更新之前运行：
  ...
  - **Redmi AX5 JDCloud**
    - 向 `feeds.conf.default` 追加 **nikki** feed（[nikkinikki-org/OpenWrt-nikki](https://github.com/nikkinikki-org/OpenWrt-nikki)，`main` 分支）—— LibWrt 默认不含该 feed，而配置启用了 `nikki` / `luci-app-nikki` / `mihomo-meta` / `luci-i18n-nikki-zh-cn`
    - 克隆 **aurora** 主题（[eamonxg/luci-theme-aurora](https://github.com/eamonxg/luci-theme-aurora)）到 `package/luci-theme-aurora`（本地以 v1.4.0 验证，克隆跟随默认分支最新提交）
  - **x86_64**：目前为空模板
