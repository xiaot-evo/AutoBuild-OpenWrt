...
- 设备专属 `devices/<name>/customize.sh`：在克隆的源码树内、feeds 更新之前运行：
  - **JDCloud AX3000**
    - 应用 `device-files/*.patch`（如 ipq50xx SKB recycler 内核配置）
    - 从 [kenzok8/small-package](https://github.com/kenzok8/small-package) 以 sparse checkout 方式仅拉取 `msd_lite` + `luci-app-msd_lite` 到 `package/`（避免克隆整个 feed 及包冲突）
    - 安装自定义主题 **luci-theme-argon**（[jerrykuku/luci-theme-argon](https://github.com/jerrykuku/luci-theme-argon)）到 `package/`，作为默认 LuCI 主题
